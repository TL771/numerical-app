...
import { CGM_main } from "../Module/CGM";
...
export default class Conjugate extends Component{
    ...
    submit(){
        ...
        const ans = CGM_main( [...this.state.A],[...this.state.B],X);
        console.log(ans);
        if(!ans.error){
            const {X} = ans
            this.setState({
    ...
    render() {
        return (
        <div>
            <h1 className="header">Conjugate Gradient Method</h1>
            <select className="form-select" value={this.state.no} onChange={this.proposChange} >
                <option key={0} value={-1} selected hidden>Select propos</option>
                {
