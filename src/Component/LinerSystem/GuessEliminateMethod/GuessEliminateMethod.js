...
import { GuessEliminateMethod } from "../Module/GEMandGJM";
...
export default class GEM extends Component{
    ...
    submit(){
        const ans = GuessEliminateMethod( [...this.state.A],[...this.state.B]);
        if(!ans.error){
            const {X} = ans
            this.setState({
                X
    ...
    render() {
        return (
        <div>
            <h1 className="header">Guess Eliminate Method</h1>
