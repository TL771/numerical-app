...
import { GSIM } from "../Module/JIMandGSIM";
import {Table} from 'react-bootstrap'
export default class Seidel extends Component{
    constructor(props){
        super(props);
        this.state = {
            size : 3,
    ...
    submit(){
        ...
        const ans = GSIM( [...this.state.A],[...this.state.B],X,Xn);
        console.log(ans);
        if(!ans.error){
            const {X} = ans
            this.setState({
    ...
    render() {
        return (
        <div>
            <h1 className="header">Gauss Seidel Iteration Method</h1>
