...
import { GaussJordanMethod } from "../Module/GEMandGJM";
import {Table} from 'react-bootstrap'
export default class GJM extends Component{
    constructor(props){
        super(props);
        this.state = {
            size : 3,
    ...
    submit(){
        const ans = GaussJordanMethod([...this.state.A],[...this.state.B]);
        if(!ans.error){
            const {X} = ans
            this.setState({
                X
    ...
    render() {
        return (
        <div>
            <h1 className="header">Gauss Jordan Method</h1>
